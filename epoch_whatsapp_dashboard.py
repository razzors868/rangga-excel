...
def epoch_whatsapp_dashboard(excel_file_path):
    st.title("Epoch Whatsapp Dashboard")

    excel_file = excel_file_path
    
    # Define required columns and data types for 'Epoch Whatsapp'
    required_cols = ['Group', 'Epoch Whatsapp', 'Date V2', 'WFH/Onsite', 'Classification', 'Inh/Vendor', 'Team leader', 'Supervisor']
    dtype_spec = {
        'Group': str,
        'Epoch Whatsapp': float, # Assuming Epoch Whatsapp is a numeric quantity
    ...
    if df.empty:
        st.warning("No data to display for Epoch Whatsapp. Please ensure the Excel file exists and contains the 'Rawdata' sheet with valid data.")
        return
    ...
    if not filtered_df.empty:
        ...
        if not pivot_table_result.empty:
            st.subheader(f"Average Epoch Whatsapp by Group ({aggregation_period} Aggregation)")
    ...
    available_chart_columns = [col for col in available_chart_columns if col not in ['Date V2', 'Epoch Whatsapp']] # Exclude Date V2 and Epoch Whatsapp
        ...
        for i in range(0, num_charts, 2):
            ...
            for j in range(2):
                if (i + j) < num_charts:
                    ...
                    with chart_cols[j]:
                        st.markdown(f"##### AVG Epoch Whatsapp by {chart_col_name}")
                        chart_data = generate_chart_data(filtered_df, aggregation_period, chart_col_name, 'Epoch Whatsapp')
                        display_altair_chart(chart_data, chart_col_name, 'Epoch Whatsapp')
    else:
        st.info("Please select columns to display dynamic charts.")

if __name__ == "__main__":
    epoch_whatsapp_dashboard()